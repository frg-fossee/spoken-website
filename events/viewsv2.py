...
from events.forms import StudentBatchForm, TrainingRequestForm, \
    ...
    OrganiserFeedbackForm, STWorkshopFeedbackForm, STWorkshopFeedbackFormPre, STWorkshopFeedbackFormPost, \
    LearnDrupalFeedbackForm, LatexWorkshopFileUploadForm, UserForm, \
...
class StudentBatchCreateView(CreateView):
    ...
    def form_valid(self, form, **kwargs):
        ...
        if 'bid' in self.kwargs:
            ...
            department = StudentBatch.objects.filter(department=form.cleaned_data['department'],
            ...
            if not this_organiser_dept.exists() and department.exists():
                messages.error(self.request, "%s department is already assigened to organiser %s in your College." % (
                form.cleaned_data['department'], department.first().organiser))
                print("form invalid: dept present ")
                return self.form_invalid(form)
            try:
                form_data = StudentBatch.objects.get(year=form_data.year, academic=form_data.academic,
        ...
        try:
            ...
            if rowcount > 500:
                messages.warning(self.request,
                                 "MB will accept only 500 students, if number is more than 500, divide the batch and upload under different departments eg. Chemistry1 & Chemistry2")
            if gencount > 500:
                messages.warning(self.request,
                                 "Total number of students per Master Batch exceeding. Masterbatch can accept maximum 500 students.")
            else:
...
class StudentBatchUpdateView(UpdateView):
    ...
    def form_valid(self, form, **kwargs):
        ...
        try:
            if str(self.sb.year) == str(form.cleaned_data['year']) and str(self.sb.department) == str(
            ...
            if (sb.exists()):
                sb = sb.first()
                messages.warning(self.request, "%s - %s Batch is already chosen by Organiser %s in your College." % (
                sb.department, sb.year, sb.organiser))
                return self.form_invalid(form)
...
class TrainingRequestCreateView(CreateView):
    ...
    def get_form_kwargs(self):
        ...
        if username.organiser.academic.institution_type.id == 5 or username.organiser.academic.institution_type.id == 13 or username.organiser.academic.institution_type.id == 15:
            ...
            ('', '---------'), (0, 'Software Course outside lab hours'), (1, 'Software Course mapped in lab hours'),
            (2, ' Software Course unmapped in lab hours'), (3, ' EduEasy Software'))})
        else:
            kwargs.update({'course_type': (
            ('', '---------'), (0, 'Software Course outside lab hours'), (1, 'Software Course mapped in lab hours'),
    ...
    def form_valid(self, form, **kwargs):
            ...
            if sb.student_count():
                ...
                if is_batch_has_course:
                    messages.error(self.request,
                                   ...
                                   form_data.batch, form_data.course))
                    return self.form_invalid(form)
        ...
        messages.success(self.request,
                         'STP has been added successfully. Now continue with step 3 "Select Participants " on STPS page. Select the participants from the Master Batch Student List for any one course that you are starting with. This is necessary for receiving certificates.')
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if self.training_request.status == 1 and not self.training_request.participants == 0:
            ...
            self.queryset = StudentMaster.objects.filter(batch_id=self.training_request.batch_id,
                                                         student__verified__lte=3, student__error=0, moved=False)
...
class TrainingCertificate(object):
    ...
    def training_certificate(self, ta):
        ...
        filename = (
                ta.student.user.first_name + '-' + ta.training.course.foss.foss + "-Participant-Certificate").replace(
            " ", "-");
        ...
        if self.role in roles and self.status in status_list:
            if self.status == 'completed':
                ...
                self.queryset = TrainingRequest.objects.filter(
                    ...
                    status=1, participants=0, training_planner__semester__name=self.prev_sem_type,
                    sem_start_date__gte=self.prev_sem_start_date
                )
            ...
            if self.status == 'completed':
                self.queryset.qs = TrainingRequestFilter(self.request.GET, queryset=self.queryset, user=self.user,
                                                         rp_completed=True)
            elif self.status == 'pending':
                self.queryset.qs = TrainingRequestFilter(self.request.GET, queryset=self.queryset, user=self.user,
                                                         rp_ongoing=True)
                ...
                self.queryset.qs = TrainingRequestFilter(self.request.GET, queryset=self.queryset, user=self.user,
                                                         rp_markcomplete=True)
            elif self.status == 'pendingattendance':
                self.queryset.qs = TrainingRequestFilter(self.request.GET, queryset=self.queryset, user=self.user,
                                                         rp_pendingattendance=True)
        else:
            raise PermissionDenied()
        return super(TrainingRequestListView, self).dispatch(*args, **kwargs)
...
class SingletrainingUpdateView(UpdateView):
    ...
    def form_valid(self, form, **kwargs):
        ...
        if 'csv_file' in self.request.FILES and self.request.FILES['csv_file']:
            ...
            skipped, error, warning, write_flag = stcv.csv_email_validate(self.request.FILES['csv_file'],
            ...
            if error or skipped:
                for i in error:
                    csv_error_line_num = (csv_error_line_num + '%d, ') % (i + 1)
                messages.error(self.request,
                               "You have error(s) in your CSV file on line numbers %s" % (csv_error_line_num))
...
class SingleTrainingListView(ListView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if is_resource_person(user) and is_organiser(user):
            self.queryset = SingleTraining.objects.filter(Q(status=status_list[self.status]),
                                                          academic__state=user.resource_person.filter(
                                                              resourceperson__status=1)).order_by('-tdate')

        elif is_organiser(user):
            self.queryset = SingleTraining.objects.filter(Q(status=status_list[self.status]),
                                                          academic__id=self.request.user.organiser.academic_id).order_by(
                '-tdate')
...
class SingleTrainingAttendanceListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        canComplete = False
        if self.single_training.training_type in [1,
                                                  2] and self.single_training.tdate <= datetime.today().date() and self.single_training.status == 3 and attendance:
            canComplete = True

        date_extn = datetime.today().date() - timedelta(days=15)
        if date_extn <= self.single_training.tdate and self.single_training.status == 3 and is_resource_person(
            self.request.user) and attendance:
            canComplete = True
...
class OldTrainingListView(ListView):
    ...
    def dispatch(self, *args, **kwargs):
        self.queryset = Training.objects.exclude(
            id__in=TrainingRequest.objects.filter(participants__gt=0).values_list('id'), ).filter(
            Q(status__gte=1) & Q(status__lte=3),
...
class OldTrainingCloseView(CreateView):
    ...
    def _create_training_planner(self, year, academic_id, organiser_id, created, even=False):
        ...
        if even:
            ...
            semester = 1
        try:
            tp = TrainingPlanner.objects.get(year=year, academic_id=academic_id, organiser_id=organiser_id,
                                             semester__even=even)
        except:
            try:
                tp = TrainingPlanner()
            ...
            if training.exists():
                ...
                try:
                    ...
                    if tdate >= even_start and tdate <= even_end:
                        tp = self._create_training_planner(year, training.academic_id, training.organiser_id,
                                                           training.tdate, even=True)
                    else:
                        tp = self._create_training_planner(year, training.academic_id, training.organiser_id,
                                                           training.tdate)
                    if tp:
...
class OrganiserFeedbackCreateView(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            messages.success(self.request,
                             "Thank you for completing this feedback form. We appreciate your input and valuable suggestions.")
            return HttpResponseRedirect(self.success_url)
        else:
            return self.form_invalid(form)
...
def payment_status(request):
    ...
    if request.method == 'POST':
        ...
        data = {'userId': user.id, 'name': user_name, 'amount': amount, 'purpose': 'Subscription',
                'channelId': 'SOLOSTW', 'random': s.hexdigest()}
        ...
        try:
            ...
            try:
                paymentdetails = PaymentDetails.objects.get(academic_id=accountexecutive.academic.id,
        ...
        if STresponsedata_hexa == random:
            ...
            try:
                ...
                messages.error(request,
                               'Something went wrong. Can not collect your transaction details. Kindly contact your state resource person.')
                return HttpResponseRedirect('/software-training')
...
def payment_details(request, choice):
    academic_id = Accountexecutive.objects.filter(user=request.user).values('academic_id',
                                                                            'academic_id__institution_name')
    ...
    context['paymentdetails'] = paymenttransactionetails.values('msg', 'refNo', 'transId', 'provId', 'status',
                                                                'created', 'paymentdetail_id__amount',
                                                                'paymentdetail_id__academic_year',
                                                                'paymentdetail_id__description')
        ...
        if status == 'O':
            ...
            if request.POST.get('fdate'):
                if request.POST.get('tdate'):
                    paymentdetails = paymentdetails.filter(
                        Q(created__gt=request.POST.get('fdate')) & Q(created__lt=request.POST.get('tdate')))
                else:
            ...
            if status in ('S', 'F'):
                paymenttransactiondetails = PaymentTransactionDetails.objects.filter(
                    paymentdetail_id__in=paymentdetails, status=str(status)).exclude(requestType='R')
            if status == 'R':
                paymenttransactiondetails = PaymentTransactionDetails.objects.filter(
                    paymentdetail_id__in=paymentdetails, requestType='R')

            if request.POST.get('fdate'):
                if request.POST.get('tdate'):
                    paymenttransactiondetails = paymenttransactiondetails.filter(
                        Q(created__gt=request.POST.get('fdate')) & Q(created__lt=request.POST.get('tdate')))
                else:
        ...
        if role in roles and status in status_list:
            if status == 'completed':
                ...
                queryset = TrainingRequest.objects.filter(
                    ...
                    status=1, participants=0, training_planner__semester__name=prev_sem_type,
                    sem_start_date__gte=prev_sem_start_date
                )
...
class StudentGradeFilter(UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if form.is_valid:
            ...
            result = self.filter_student_grades(foss, state, city, grade, institution_type, activation_status,
                                                from_date, to_date)
        return self.render_to_response(self.get_context_data(form=form, result=result))

    def filter_student_grades(self, foss=None, state=None, city=None, grade=None, institution_type=None,
                              activation_status=None, from_date=None, to_date=None):
        if grade:
            ...
            try:
                ...
                user_grade = MdlQuizGrades.objects.using('moodle').values_list('userid', 'quiz', 'grade').filter(
                    quiz__in=[f.mdlquiz_id for f in fossmdl], grade__gte=int(grade))
